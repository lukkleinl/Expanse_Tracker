...
public class SWE_Observable {
   ...
   * @throws CustomException if Observer already subscribed.
   */
  public void subscribe(final SWE_Observer obs) throws CustomException {
    if (this.observerList.contains(obs))
      throw new CustomException(
          "Observable ALREADY SUBSCRIBED to this Observer - can not subscribe!");
    else {
   ...
   * @throws CustomException if Observer not subscribed at this time.
   */
  public void unSubscribe(final SWE_Observer obs) throws CustomException {
    if (!this.observerList.contains(obs))
      throw new CustomException(
          "Observable was NOT SUBSCRIBED to this Observer - can not unsubscribe!");
    else {
  ...
  /**
   * Different updateObservers because of usage Doesnt need to update both collections users and
  ...
  public void updateObservers(final User user) {
    for (SWE_Observer obs : this.observerList) {
      obs.update(user);
    }
  }
